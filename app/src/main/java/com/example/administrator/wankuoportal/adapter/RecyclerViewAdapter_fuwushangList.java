...
/**
 * Created by lv
 * on 2017/9/26.  服务商adapter
 */
public class RecyclerViewAdapter_fuwushangList extends RecyclerView.Adapter<RecyclerViewAdapter_fuwushangList.ViewHolder> implements View.OnClickListener {
    ...
    public List<SearchFuwushang.DatasBean> getList(){
        return dataList;
    }
    ...
    public RecyclerViewAdapter_fuwushangList(Context context) {
        mContext = context;
    }
...
//
        ...
        if (dataList.get(position).getAshopData().getLevel() == 0) {
            holder.shoplevel.setText("级别：普通会员  保证金：0  好评率：" + dataList.get(position).getAshopData().getEvaluate() + "%");
            holder.isbaozhang.setVisibility(View.VISIBLE);
        } else if (dataList.get(position).getAshopData().getLevel() == 1) {
            holder. isbaozhang.setVisibility(View.VISIBLE);
            holder.shoplevel.setText("级别：白金会员  保证金：5000  好评率：" + dataList.get(position).getAshopData().getEvaluate() + "%");
        } else if (dataList.get(position).getAshopData().getLevel() == 2) {
            holder.isbaozhang.setVisibility(View.VISIBLE);
            holder.shoplevel.setText("级别：钻石会员  保证金：10000  好评率：" + dataList.get(position).getAshopData().getEvaluate() + "%");
        } else if (dataList.get(position).getAshopData().getLevel() == 3) {
            holder.isbaozhang.setVisibility(View.VISIBLE);
            holder.shoplevel.setText("级别：皇冠会员  保证金：20000  好评率：" + dataList.get(position).getAshopData().getEvaluate() + "%");
        } else if (dataList.get(position).getAshopData().getLevel() == 4) {
            holder.isbaozhang.setVisibility(View.VISIBLE);
            holder.shoplevel.setText("级别：尊贵会员  保证金：69800  好评率：" + dataList.get(position).getAshopData().getEvaluate() + "%");
        } else if (dataList.get(position).getAshopData().getLevel() == 5) {
            holder.isbaozhang.setVisibility(View.VISIBLE);
            holder.shoplevel.setText("级别：至尊会员  保证金：109800  好评率：" + dataList.get(position).getAshopData().getEvaluate() + "%");
        }
